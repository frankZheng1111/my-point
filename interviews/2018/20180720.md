...
- 直接结论:
  ...
  - [explain的字段意义](https://blog.csdn.net/lixingying567/article/details/73505943): 
    ...
    - table: 输出结果的表
    - type: 表示MySql在表中找到所需行的方式，或者叫访问类型。常见的类型：ALL(全表扫描); index(遍历整个索引); range(索引范围扫描: >, <,between in 等); ref(非唯一索引扫描); eq_ref(用于多表连接 主建/唯一健作为关联条件); const/system(唯一索引字段=) NULL(没有用到索引: 如limit0)
